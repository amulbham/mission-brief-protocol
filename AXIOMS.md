...
**The Kernel — Mission Brief Protocol**
...
```
The sealed output is the only thing visible to the user.
Capsule internals remain hidden unless explicitly requested.
```

Sealed output includes: the response, mini-map, and header — all produced post-closure from verified state. Nothing from inside the reasoning process surfaces unbidden.
...
**Why this is constitutional:** Axiom I governs the input boundary — cognition must be inside capsules. The Output Rule governs the output boundary — only sealed verified output crosses back to the user. Together they close both ends of the capsule.
...
```
WHERE cognition happens  →  Axiom I   (containment)
HOW cognition is recorded → Axiom II  (transparency)
WHAT cognition commits to → Axiom III (verification)
WHAT the user receives    → Output Rule (sealed output only)
```

Four constitutional statements. The entire MBP operating system is consequence.

---

## On Axiom Discovery

These axioms were not designed top-down. They were discovered through six months of stress testing a framework that began with four fields. Each axiom surfaced when the architecture revealed a failure mode that no existing rule could systematically close.

Axiom I was implicit from the beginning — the capsule was always the container. Axiom II emerged when silent self-correction was identified as a structural vulnerability. Axiom III was the last to be formally named, when the distinction between Vgate governing intake and Logic Debugger governing reasoning made clear that a third constitutional constraint was load-bearing and missing.

The kernel was always there. Stress revealed it.
